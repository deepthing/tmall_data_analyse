...
-- 区域分析
select area_info 区域,SUM(order_number) 数量,SUM(order_amount) 金额 from t_order_area
where 1=1
...
where 1=1
and  t_settle_amount_info.period like '2017-06%' ;
...
t_settle_fee_info
where 1=1
and  t_settle_fee_info.fee_time like '201706%' ;



-- 订单金额分析（订单生成时间）
select 
SUM(tmall_refund+actual_paid),
SUM(tmall_refund),
SUM(actual_paid),
SUM(order_fee*-1),
SUM(alipay_get*-1)
...
SUM(t.juhuasuan)
from t_fee_info t
where payment_time like '2017-06%' ;


-- -- 月度金额分析
-- select SUBSTR(fin_period FROM 1 FOR 7),sum(actual_paid),sum(alipay_actual_recieve),
-- sum(refund*-1),sum(order_fee*-1),sum(alipay_get*-1), sum(account_fee)
-- from t_monthly_order_amount where SUBSTR(fin_period FROM 1 FOR 7) = '2017-02' 
-- group BY SUBSTR(fin_period FROM 1 FOR 7);
-- 
-- -- 月度费用明细
-- select sum(logisitic_tax),sum(logisitic_service),sum(alipay_service),sum(tmall),sum(juhuasuan) 
-- from t_fee_monthly_info where fee_time like '2017-01%';
-- 
-- 
-- 账户维度
select 
...
where 1=1
and  t_myaccount_monthly_info.period like '2017-06%' ;
...
-- 总表
select * from t_period_num_info pn
where 1=1
-- AND pn.goods_id='XY521067771349'
ORDER BY pn.goods_id,pn.fee_order
;

-- 总表总计，不算椰果
select fee_order,fee_type,
SUM(P201509),
SUM(P201510),
SUM(P201511),
SUM(P201512),
